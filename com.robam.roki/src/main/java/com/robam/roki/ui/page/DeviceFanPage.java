...
import com.robam.roki.ui.view.FanCtr8700View;
import com.robam.roki.ui.view.FanCtr9700View;
...
//        }
        ...
        } else if (DeviceTypeManager.getInstance().isInDeviceType(fan.getGuid(), IRokiFamily.R9700)) {
            ctrView = new FanCtr9700View(cx);
        } else if (DeviceTypeManager.getInstance().isInDeviceType(fan.getGuid(),IRokiFamily.R8229)){
            ctrView = new FanCtr8229View(cx);
        }

        Preconditions.checkNotNull(ctrView, "invalid fan, no matched view");
        ctrView.attachFan((IFan)fan);
