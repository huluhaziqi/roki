...
import com.robam.roki.ui.dialog.StoveCtrNoticeDialog;
 ...
 */
public class StoveCtr9w70View_new extends FrameLayout implements UIListeners.IStoveCtrView {
    ...
    public StoveCtr9w70View_new(Context cx){
        super(cx);
        init(cx, null);

    }

    public StoveCtr9w70View_new(Context cx, AttributeSet attrs) {
        super(cx, attrs);
    }

    void init(Context cx,AttributeSet attrs){
        View view = LayoutInflater.from(cx).inflate(R.layout.view_9w70_new,
                this, true);
    ...
    @OnClick(R.id.divStoveHead)
    public void onClickdivStoveHead(){
        StoveCtrNoticeDialog.show(this.getContext());
    }
...
//    }
    ...
    public void refreshHead(Stove.StoveHead head) {
        ...
        if (head == null || !stove.isConnected()) {
            ...
            setViewValid(head,false);
            if (stove.leftHead.status==StoveStatus.Working||stove.rightHead.status==StoveStatus.Working){
